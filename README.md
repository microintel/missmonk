...
Missmonk is a web app (`index.html` + a small serverless API) backed by Firebase and Vercel, plus a companion swipeable feed (`reels.html`, internally called **"Nuts"**). Together they cover:
...
- **Search** across every lesson, book, folder, and course — resolved server-side via the API so the browser never has to download the full curriculum just to search it — plus a PWA manifest so it can be installed like a native app.
- **Shareable, URL-synced navigation** — the address bar reflects exactly where you are (`?course=...&path=...`) as you browse, so links can be copied/shared and the browser's back/forward buttons work as expected.
...
- A solo builder or small group who wants a **content structure they fully control** (everything lives in one editable `data.json`, served through a handful of small API files) rather than a black-box platform.
...
- **Anyone needing offline-first or fully self-hosted use.** The app depends on live Firebase (Auth + Firestore), a Vercel-hosted serverless API for curriculum data, and several CDN scripts (fonts, Bootstrap Icons, Chart.js, SweetAlert2); without internet access and a working Firebase project + Vercel deployment, login, content loading, and sync won't function. Note the app must be served **from Vercel** — a static host like GitHub Pages can serve `index.html` but can't run the `/api/*` routes it depends on, so content will hang on "loading."
...
- **Vercel serverless functions** (`/api/curriculum/*`) serve the curriculum lazily and power search, so the client only ever downloads what it's currently viewing instead of the entire `data.json`:
  - `api/curriculum/skeleton.js` — tiny first response: book metadata + every top-level course collapsed into a stub.
  - `api/curriculum/node.js` — expands one course/folder's immediate children on demand.
  - `api/curriculum/search.js` — searches the whole tree server-side.
  - `api/curriculum/full.js` — the complete curriculum, used by the Books and Progress pages, which need every lesson at once for totals.
  - `api/_lib/data.js` — shared helpers (path resolution, lazy-stub shaping, search) used by all of the above.
...
- `data.json` (repo root) is the single source of truth for all curriculum content (courses → folders → lessons) and book metadata — read only by the API, never shipped to the client in full.
- Ships with a `manifest.json` reference, making it installable as a PWA on mobile/desktop.

## Deployment

The app is designed to run on **Vercel** — currently live at [microintel-missmonk.vercel.app](https://microintel-missmonk.vercel.app/). The repo layout matters because the API's relative `require()` paths depend on it:
...
The repo is also mirrored on GitHub Pages for convenience, but GitHub Pages is static-only and can't run the `/api/*` routes — `index.html` includes a small redirect snippet that bounces any `github.io` visitor straight to the Vercel deployment (preserving the current path and query string) so shared links keep working either way.
